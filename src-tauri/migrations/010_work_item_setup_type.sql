...
CREATE TABLE work_items (
    ...
    work_item_type TEXT NOT NULL DEFAULT 'feature' CHECK(work_item_type IN ('feature','setup','bug','refactor','test','review','security_fix','performance_improvement')),
...
ALTER TABLE workflow_runs RENAME TO workflow_runs_fix_legacy;
...
INSERT INTO workflow_runs (
    ...
    coordinator_agent_id, pending_stage_name, retry_count, max_retries,
    error_message, started_at, ended_at, updated_at
)
SELECT
    id, work_item_id, workflow_version, status, current_stage, assigned_team_id,
    coordinator_agent_id, pending_stage_name, retry_count, max_retries,
    error_message, started_at, ended_at, updated_at
FROM workflow_runs_fix_legacy;

DROP TABLE workflow_runs_fix_legacy;
CREATE INDEX IF NOT EXISTS idx_workflow_runs_work_item ON workflow_runs(work_item_id);

ALTER TABLE agent_runs RENAME TO agent_runs_fix_legacy;
...
SELECT
    ...
    prompt_snapshot_path, output_snapshot_path, token_count_input, token_count_output,
    duration_ms, error_message, started_at, ended_at, created_at
FROM agent_runs_fix_legacy;

DROP TABLE agent_runs_fix_legacy;
CREATE INDEX IF NOT EXISTS idx_agent_runs_workflow ON agent_runs(workflow_run_id);

ALTER TABLE approvals RENAME TO approvals_fix_legacy;
...
SELECT
    id, work_item_id, workflow_run_id, approval_type, status, notes, acted_at, created_at
FROM approvals_fix_legacy;

DROP TABLE approvals_fix_legacy;
CREATE INDEX IF NOT EXISTS idx_approvals_work_item ON approvals(work_item_id);

ALTER TABLE artifacts RENAME TO artifacts_fix_legacy;
...
SELECT
    id, work_item_id, workflow_run_id, agent_run_id, artifact_type, storage_path,
    summary, content_type, size_bytes, created_at
FROM artifacts_fix_legacy;

DROP TABLE artifacts_fix_legacy;
...
ALTER TABLE findings RENAME TO findings_fix_legacy;
...
INSERT INTO findings (
    id, work_item_id, source_agent_run_id, category, severity, title,
    description, status, is_blocking, linked_followup_work_item_id, created_at, updated_at
)
SELECT
    id, work_item_id, source_agent_run_id, category, severity, title,
    description, status, is_blocking, linked_followup_work_item_id, created_at, updated_at
FROM findings_fix_legacy;

DROP TABLE findings_fix_legacy;
