...
mod tests {
    ...
    async fn migration_43_upgrades_legacy_module_physical_schema() {
        let (legacy_pool, database_url) =
            create_unmigrated_test_pool("legacy_module_upgrade").await;
        ...
        let capability_product_area_id: String = sqlx::query_scalar(
            "SELECT product_area_id FROM capabilities WHERE id='legacy-capability'",
        )
        .fetch_one(&upgraded_pool)
        .await
